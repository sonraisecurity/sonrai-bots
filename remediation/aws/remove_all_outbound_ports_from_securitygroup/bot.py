...
# Remove outbound rules from a security group : AWS ref(https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/ec2.html#EC2.Client.revoke_security_group_egress)
...
def run(ctx):
  ...
  # Step 5) Get security group details from EC2 instance and remove all outbound port/protocol from security group
    ...
    for instance in reservations['Instances']:
      for security_group in instance['SecurityGroups']:
        ...
        for security_group_detail in security_group_response['SecurityGroups']:
          ip_permissions = security_group_detail['IpPermissionsEgress']
          if(len(ip_permissions)==0):
            log.warning("[{}] No outbound port/protocol to remove from security group".format(security_group_id))
          else:
            log.info("[{}] Removing all outbound port/protocol from security group".format(security_group_id))
            client.revoke_security_group_egress(GroupId=security_group_id, IpPermissions=ip_permissions)
            log.info("[{}] Removed all outbound port/protocol from security group".format(security_group_id))
